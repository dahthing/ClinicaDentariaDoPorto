...
(function(window,undefined){
	...
	$(function(){
		...
		$('<span class="ajaxify-loader icon-spin icon-refresh icon-4x hide"></span>').appendTo('body');
		$('.ajaxify-loader').css({
			'top': $(window).height()/2,
			'marginLeft': -$('.ajaxify-loader').width()/2,
			'marginTop': -$('.ajaxify-loader').height()/2
		});
		...
		$window.bind('statechange',function(){
			...
			$.ajax({
				...
				success: function(data, textStatus, jqXHR){
					...
					// Fetch the stylesheets/less
					$less = $dataHead.find('link[rel="stylesheet/less"]');
					if ($less.length)
						$less.detach();
					...
					var lessLoaded = typeof less != 'undefined';
					if (lessLoaded)
						less.sheets = [];

					$less.each(function(k,v){
						$(v).insertAfter('head > meta:last');
						if (lessLoaded)
							less.sheets.push(v);
					});
					if (lessLoaded)
